...
# Mapa simples de nomes de cor em português -> hex, usado para converter a seção
# "Cores da marca" do skill em cores utilizáveis na sobreposição de texto da imagem.
CORES_PT = {
    "preto": "#000000",
    "branco": "#FFFFFF",
    "amarelo": "#FFD400",
    "azul petróleo": "#0B4F6C",
    "azul": "#0B4F6C",
    "vermelho": "#D32F2F",
    "verde": "#2E7D32",
    "laranja": "#F57C00",
...
def carregar_skill(cliente: str) -> dict:
    ...
    return {
        ...
        "cores_hex": _cores_marca(texto),
    }


def _cores_marca(texto: str) -> list:
    """Extrai as cores da seção 'Cores da marca' (ou 'Cores') do skill, convertendo
    nomes em português para hex via CORES_PT. Usado na sobreposição de texto da imagem;
    se nada for reconhecido, o chamador aplica um fallback neutro."""
    secao = _secao(texto, "Cores da marca") or _secao(texto, "Cores")
    itens = re.findall(r"^\s*-\s*(.+)$", secao, re.MULTILINE)
    cores = []
    for item in itens:
        chave = item.strip().lower()
        if chave in CORES_PT:
            cores.append(CORES_PT[chave])
    return cores

...
if st.session_state.get("prompt_imagem"):
    with st.container(border=True):
        ...
        if imagem_info:
            ...
            if imagem_info.get("imagem_b64"):
                with st.form(f"form_ajuste_{cliente}"):
                    instrucao_ajuste = st.text_input(
                        "🔧 Pedir uma alteração nesta imagem",
                        placeholder="Ex: deixe o céu ao entardecer / troque a cor da faixa pra azul petróleo",
                    )
                    aplicar_ajuste = st.form_submit_button("🪄 Aplicar alteração")
                st.caption(
                    "Usa o modelo de edição da OpenAI (gpt-image-2.5-sunburst), mantendo "
                    "o resto da imagem fiel ao original. Cada ajuste é mais uma chamada "
                    "de imagem (gasta crédito)."
                )
                if aplicar_ajuste and instrucao_ajuste.strip():
                    with st.spinner("Aplicando o ajuste..."):
                        try:
                            imagem_atual_bytes = base64.b64decode(imagem_info["imagem_b64"])
                            resultado_ajuste = ajustar_imagem(imagem_atual_bytes, instrucao_ajuste.strip(), skill)
                            if resultado_ajuste.get("imagem_b64"):
                                st.session_state.imagem = resultado_ajuste
                                st.session_state.imagem_erro = None
                                pasta = _pasta_saida(cliente)
                                (pasta / "imagem.png").write_bytes(
                                    base64.b64decode(resultado_ajuste["imagem_b64"])
                                )
                            else:
                                st.session_state.imagem_erro = "A API não retornou imagem nesse ajuste."
                        except Exception as exc:  # noqa: BLE001 — mostramos o erro real na tela
                            st.session_state.imagem_erro = str(exc)
                    st.rerun()

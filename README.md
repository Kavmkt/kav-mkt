...
- A geração inicial da imagem é numa única tentativa — se não gostar do resultado geral
  (composição/tema), a forma de "tentar de novo" é gerar um novo post inteiro. Mas dá
  pra pedir **ajustes pontuais** na imagem já gerada (ver seção abaixo), sem precisar
  gerar tudo de novo.

## Pedir ajustes numa imagem já gerada

Depois que a imagem aparece na tela, um campo **"🔧 Pedir uma alteração nesta imagem"**
deixa você descrever um ajuste pontual (ex: "deixe o céu ao entardecer", "troque a cor
da faixa pra azul petróleo") e clicar em **"🪄 Aplicar alteração"**. Isso chama o
`gpt-image-2.5-sunburst` (o modelo de edição indicado pela OpenAI para manter
fidelidade), usando a imagem atual como referência — o resto da composição tende a ficar
igual, só o que foi pedido muda. Cada ajuste é uma chamada de imagem a mais (gasta
crédito), e substitui a imagem exibida (a versão anterior não fica salva à parte, exceto
se você já tiver baixado o PNG).
